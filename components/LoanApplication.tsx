...
const LoanApplication: React.FC<LoanApplicationProps> = ({ user, loans, systemBudget, isGlobalProcessing, onApplyLoan, onSettleLoan, onBack, initialLoanToSettle, initialLoanToView }) => {
  ...
  const handleConfirmSettlement = async () => {
    if (settleLoan && billImage && !isSubmitting && !isGlobalProcessing) {
      ...
      setIsSubmitting(true);
      try {
        await onSettleLoan(settleLoan.id, billImage, settleType, bankTransactionId, settleType === 'PARTIAL' ? partialAmount : undefined);
    ...
    return (
      <div className="fixed inset-0 z-[100] bg-black animate-in slide-in-from-right duration-300 flex flex-col p-4 overflow-hidden">
        ...
        <div className="mt-1 flex-none">
          ...
          >
            {isSubmitting || isGlobalProcessing ? 'ĐANG XỬ LÝ...' : (isDuplicateTransaction ? 'MÃ GIAO DỊCH ĐÃ TỒN TẠI' : (billImage ? 'GỬI XÉT DUYỆT NGAY' : 'VUI LÒNG ĐÍNH KÈM BILL'))}
          </button>
        </div>
      </div>
    );
