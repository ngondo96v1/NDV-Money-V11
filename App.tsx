...
const App: React.FC = () => {
  ...
  const handleAdminLoanAction = async (loanId: string, action: 'APPROVE' | 'DISBURSE' | 'SETTLE' | 'REJECT', reason?: string) => {
    ...
    try {
      ...
      const updatedLoan = { 
        ...loan, 
        status: (action === 'SETTLE' && (loan.settlementType === 'PRINCIPAL' || loan.settlementType === 'PARTIAL')) ? 'ĐANG NỢ' : (newStatus as any), 
        date: newDueDate,
        amount: (action === 'SETTLE' && loan.settlementType === 'PARTIAL') ? (loan.amount - (loan.partialAmount || 0)) : loan.amount,
        rejectionReason, 
        // Tăng số lần gia hạn nếu là Tất toán Gốc hoặc TTMP thành công
        principalPaymentCount: (action === 'SETTLE' && (loan.settlementType === 'PRINCIPAL' || loan.settlementType === 'PARTIAL')) 
          ? (loan.principalPaymentCount || 0) + 1 
          : loan.principalPaymentCount,
        // Clear bill and type if it's a successful Principal/Partial Settlement to keep the next cycle clean
        billImage: (action === 'SETTLE' && (loan.settlementType === 'PRINCIPAL' || loan.settlementType === 'PARTIAL')) ? null : loan.billImage,
        settlementType: (action === 'SETTLE' && (loan.settlementType === 'PRINCIPAL' || loan.settlementType === 'PARTIAL')) ? null : loan.settlementType,
        partialAmount: (action === 'SETTLE' && (loan.settlementType === 'PRINCIPAL' || loan.settlementType === 'PARTIAL')) ? null : loan.partialAmount,
        fine: (action === 'SETTLE' && (loan.settlementType === 'PRINCIPAL' || loan.settlementType === 'PARTIAL')) ? 0 : loan.fine,
        updatedAt: Date.now() 
      };
